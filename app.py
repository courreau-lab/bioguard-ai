...
try:
    if "GEMINI_API_KEY" in st.secrets:
        ...
        st.warning("⚠️ AI Offline: Add GEMINI_API_KEY in Streamlit Secrets.")
...
# --- 6. PROTECTED PAGES ---
if st.session_state.logged_in:
    with tabs[3]: # ANALYSIS ENGINE (QUOTA & SIZE GUARD)
        st.header("🎥 Live AI Technical Audit")
        vid_file = st.file_uploader("Upload Match Clip (Max 20MB)", type=['mp4', 'mov'])
        if vid_file and 'client' in locals():
            st.video(vid_file)
            # CHECK FILE SIZE
            if vid_file.size > 20 * 1024 * 1024:
                st.error("❌ Video too large for Free Tier. Please upload a clip under 20MB.")
            else:
                elapsed = time.time() - st.session_state.last_ai_time
                if elapsed < 60:
                    st.warning(f"🕒 AI Cooldown: Wait {int(60 - elapsed)}s for next analysis.")
                else:
                        ...
                        with st.status("🤖 Clearing storage and analyzing..."):
                            try:
                                ...
                                with tempfile.NamedTemporaryFile(delete=False, suffix='.mp4') as tmp:
                                    tmp.write(vid_file.getvalue()); t_path = tmp.name
                                up_f = client.files.upload(file=t_path)
                                resp = client.models.generate_content(model="gemini-2.0-flash-exp", contents=["Analyze.", up_f])
                                st.session_state.roadmap["22"].append({"date": "2026-01-18", "category": "AI", "note": resp.text})
                                st.session_state.last_ai_time = time.time()
                                client.files.delete(name=up_f.name); os.remove(t_path)
                                st.success("Analysis Delivered.")
                            except Exception:
                                st.error("Quota reached. Wait 60s for reset.")
    ...
    with tabs[4]: # PLAYER DASHBOARD (ALIGNMENT FIX)
        ...
        if os.path.exists("digital_twin.png"):
            ...
            # RECALIBRATED MIDLINE ALIGNMENT
            fig.add_trace(go.Scatter(
                x=[500, 500], y=[235, 125], mode='markers+text', 
                text=["Knee ACL", "Calf Strain"], textposition="middle right",
