...
def apply_elite_styling():
    ...
    <style>
        ...
        /* FORCE VISIBILITY: Dark backgrounds, Green labels, White text */
        ...
        /* INPUT BOXES: Black background, white text, green border (Fixes the 'White on White' bug) */
...
# MASTER DATABASE: Fixed the dictionary nesting error from image_f85083.jpg
...
with tabs[0]: # OVERVIEW (KeyError Fix)
    ...
    # Table logic with .get() to prevent the KeyError from image_99ed69.jpg
...
with tabs[1]: # SQUAD REGISTRY
    ...
    with st.form("registry_form_final", clear_on_submit=True):
...
with tabs[2]: # MEDICAL HUB
    ...
    p_uid = st.selectbox("Select Athlete Profile", options=list(p_options.keys()), format_func=lambda x: p_options[x], key="med_main")
    
    col_l, col_r = st.columns([1, 1.5])
    with col_l:
        st.markdown("<div class='luxury-card'>", unsafe_allow_html=True)
        st.subheader(f"Update: {st.session_state.profiles[p_uid]['first_name']}")
        ...
        r = st.selectbox("Injury Risk", ["Low", "Medium", "High"], key="risk_update")
    ...
    with col_r:
        if os.path.exists("digital_twin.png"):
            ...
            fig = go.Figure()
            # ValueError Fix: Cleaned dict syntax for plotly
...
with tabs[3]: # VIRTUAL COACH
    ...
    c_uid = st.selectbox("Session Target", options=list(p_options.keys()), format_func=lambda x: p_options[x], key="coach_main")
    vid = st.file_uploader("Upload Session Video", type=['mp4', 'mov'], key="coach_vid_final")
    if vid and 'client' in locals():
        st.video(vid)
        if st.button("Begin Technical Audit"):
            with st.status("🤖 Analyzing MM:SS timestamps..."):
                try:
                    for f in client.files.list(): client.files.delete(name=f.name)
                    ...
                    prompt = f"Target shirt #{st.session_state.profiles[c_uid]['shirt_number']}. Provide touches, passes, tactical errors with MM:SS timestamps, and body form analysis."
...
with tabs[5]: # ROADMAP
    ...
    r_uid = st.selectbox("View History", options=list(p_options.keys()), format_func=lambda x: p_options[x], key="road_main")
