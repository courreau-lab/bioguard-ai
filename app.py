...
# --- 2. SESSION SECURITY & LOGIN ---
if "logged_in" not in st.session_state:
    st.session_state.logged_in = False

# MASTER DATABASE: Structured to prevent crashes
...
if not st.session_state.logged_in:
    ...
    p = st.text_input("Password", type="password", placeholder="owner2026", key="login_password")
...
with tabs[2]: # MEDICAL HUB
    ...
    p_uid = st.selectbox("Select Profile", options=list(p_options.keys()), format_func=lambda x: p_options[x], key="med_main")
        ...
        if st.button("Generate Tactical & Form Audit"):
            with st.status("🤖 Analyzing metrics and timestamps..."):
                try:
                    ...
                    prompt = f"Target shirt #{st.session_state.profiles[c_uid]['shirt_number']}. Provide touches, passes, MM:SS timestamps for errors, and body form efficiency."
