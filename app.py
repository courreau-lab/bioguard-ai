...
if "roadmap" not in st.session_state:
    st.session_state.roadmap = {
        "2": [{"date": "2026-01-11", "issue": "Right Knee", "note": "Gait asymmetry - Week 2 Rehab active"}]
    }

# --- 3. THE NAVIGATION MENU ---
# Accessible even without login
...
with current_tab[0]: # HOME
    ...
    st.subheader("Professional Biomechanics & AI-Driven Injury Prevention")
    st.write("Democratizing the elite medical standards of the 1% for professional academies and clubs.")
...
with current_tab[1]: # BUSINESS OFFER
    ...
    st.write("Elite Performance bridges the gap between matchday video and clinical physiological data.")
    ...
    with col2:
        ...
        st.write("- Reduce non-contact injuries by up to 22%\n- Protect player market valuation\n- Automated 12-week recovery roadmaps")

with current_tab[2]: # SUBSCRIPTION
    st.header("Subscription Plans")
    p1, p2, p3 = st.columns(3)
    p1.markdown("<div class='luxury-card'><h3>Individual</h3><h2>£29/mo</h2><p>1 Player Scan<br>Bi-Weekly Report</p></div>", unsafe_allow_html=True)
    p2.markdown("<div class='luxury-card' style='border-color: #00ab4e'><h3>Squad Pro</h3><h2>£199/mo</h2><p>Full Team (25 Players)<br>Interactive Body Map<br>Daily AI Audits</p></div>", unsafe_allow_html=True)
    p3.markdown("<div class='luxury-card'><h3>Elite Academy</h3><h2>£POA</h2><p>Full Club Integration<br>Custom 3D Mapping<br>API Access</p></div>", unsafe_allow_html=True)

# --- 5. PRIVATE MEMBER CAPABILITIES (Locked) ---

if st.session_state.logged_in:
    with current_tab[3]: # ANALYSIS ENGINE
        st.header("🎥 Video Analysis Engine")
        st.write("Upload match highlights to detect mechanical drift and injury risks.")
        p_num = st.text_input("Target Player Number", "2")
        video_file = st.file_uploader("Upload Video Highlight", type=['mp4', 'mov'])
        ...
        if video_file:
            ...
            if st.button("Generate Elite Audit"):
                with st.status("Analyzing Skeletal Landmarks..."):
                    time.sleep(3)
                    # Update roadmap without deleting
                    findings = "Detected 12° Knee Valgus. MRI Scan Recommended to rule out Grade 1 ACL."
    ...
    with current_tab[5]: # 12-WEEK ROADMAP
        ...
        p_select = st.selectbox("View History for Player", list(st.session_state.roadmap.keys()), key="roadmap_p")
        
        for event in reversed(st.session_state.roadmap[p_select]):
            st.markdown(f"**{event['date']}**: {event['note']}")
        
        st.divider()
        st.warning("⚠️ RECOVERY ADVICE: Based on the latest scan, incorporate Week 3 Proprioception drills. Consult clinical staff if valgus exceeds 10° in next match.")
